...
Public Module Startup
    ...
    Public Sub Main()
        ...
        RemotingConfiguration.ApplicationName = "TalkServer"
        Dim Channel As New PeerChannel()
        ChannelServices.RegisterChannel(Channel)

        Dim Uri As String = "ServerObject"
        Dim ServiceEntry As New WellKnownServiceTypeEntry(GetType(ServerProcess), Uri, WellKnownObjectMode.Singleton)

        ''''
        'PeerChannel.SecureWellKnownServiceType(ServiceEntry)

        RemotingConfiguration.RegisterWellKnownServiceType(ServiceEntry)
        Dim Url As String = PeerChannel.GetUrl(Uri)
        Trace.WriteLine(Url)
